...
// Fallback file loader when CubismFramework::Option.LoadFileFunction is not set
#if defined(CSM_TARGET_ANDROID_ES2)
#include "../../../Live2DPal.hpp"
#define CSM_FALLBACK_FILE_LOADER ::Live2DPal::LoadFileAsBytes
#define CSM_FALLBACK_BYTES_RELEASER ::Live2DPal::ReleaseBytes
#endif
...
void CubismShader_OpenGLES2::GenerateShaders()
{
    ...
    // 遅延コンパイル用の登録領域を用意する（未登録 = Vert が nullptr）
    _shaderSpecs.Clear();
    for (csmInt32 i = 0; i < ShaderNames_ShaderCount; i++)
    {
        ShaderSpec spec;
        spec.Vert = nullptr;
        spec.Frag = nullptr;
        spec.ColorBlend = ColorBlendMode_None;
        spec.AlphaBlend = AlphaBlendMode_None;
        spec.Mask = MaskType_None;
        spec.IsBlend = false;
        _shaderSpecs.PushBack(spec);
    }
    ...
    else
    {
        ...
        // ブレンドモードの組み合わせ分（16 × 5 × 6 = 480 個）はここではコンパイルしない。
        // 起動時に全部コンパイルすると実機で 4 秒以上かかるので、実際に使う組み合わせだけを
        // GetShaderSet() で遅延コンパイルする（登録は下の SetShaderSet ループ側で行う）。
    }
    ...
    // ブレンドモードの組み合わせ分は、ここではコンパイルせずに必要な情報だけ登録しておく。
    // 実際のコンパイルは GetShaderSet() が初回使用時に行う。
    {
        csmUint32 offset = ShaderNames_NormalAtop;
        for (csmInt32 i = 0; i < ColorBlendMode_Count; ++i)
        {
            ...
            {
                //
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcBlend.vert", "FrameworkShaders/FragShaderSrcBlend.frag", i, j, MaskType_None, true);
                // クリッピング
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcMaskedBlend.vert", "FrameworkShaders/FragShaderSrcMaskBlend.frag", i, j, MaskType_Masked, true);
                // クリッピング・反転
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcMaskedBlend.vert", "FrameworkShaders/FragShaderSrcMaskInvertedBlend.frag", i, j, MaskType_MaskedInverted, true);
                // PremultipliedAlpha
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcBlend.vert", "FrameworkShaders/FragShaderSrcPremultipliedAlphaBlend.frag", i, j, MaskType_PremultipliedAlpha, true);
                // クリッピング、PremultipliedAlpha
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcMaskedBlend.vert", "FrameworkShaders/FragShaderSrcMaskPremultipliedAlphaBlend.frag", i, j, MaskType_MaskedPremultipliedAlpha, true);
                // クリッピング・反転、PremultipliedAlpha
                RegisterShader(offset++, "FrameworkShaders/VertShaderSrcMaskedBlend.vert", "FrameworkShaders/FragShaderSrcMaskInvertedPremultipliedAlphaBlend.frag", i, j, MaskType_MaskedInvertedPremultipliedAlpha, true);
            }
        }
    }
}

void CubismShader_OpenGLES2::RegisterShader(csmInt32 index, const csmChar* vert, const csmChar* frag,
                                            csmInt32 colorBlend, csmInt32 alphaBlend,
                                            MaskType mask, csmBool isBlend)
{
    if (index < 0 || index >= static_cast<csmInt32>(_shaderSpecs.GetSize()))
    {
        return;
    }
    ShaderSpec& spec = _shaderSpecs[index];
    spec.Vert = vert;
    spec.Frag = frag;
    spec.ColorBlend = colorBlend;
    spec.AlphaBlend = alphaBlend;
    spec.Mask = mask;
    spec.IsBlend = isBlend;
}

CubismShader_OpenGLES2::CubismShaderSet* CubismShader_OpenGLES2::GetShaderSet(csmInt32 index)
{
    CubismShaderSet* shaderSet = _shaderSets[index];
    if (shaderSet->ShaderProgram != 0)
    {
        return shaderSet;
    }
    if (index < 0 || index >= static_cast<csmInt32>(_shaderSpecs.GetSize()))
    ...
    const ShaderSpec& spec = _shaderSpecs[index];
    if (spec.Vert == nullptr)
    {
        return shaderSet;
    }

    // 初めて使う組み合わせなので、ここでコンパイルして変数のアドレスを拾い直す
    shaderSet->ShaderProgram = LoadShaderProgramFromFile(spec.Vert, spec.Frag, spec.ColorBlend, spec.AlphaBlend);
    if (shaderSet->ShaderProgram != 0)
    {
        SetShaderSet(*shaderSet, spec.Mask, spec.IsBlend);
    }
    return shaderSet;
}
...
void CubismShader_OpenGLES2::SetupShaderProgramForDrawable(CubismRenderer_OpenGLES2* renderer, const CubismModel& model, const csmInt32 index)
{
    ...
    CubismShaderSet* shaderSet = GetShaderSet(shaderNameBegin + offset);
...
#ifdef CSM_FALLBACK_FILE_LOADER
    if (!fileLoader)   fileLoader    = CSM_FALLBACK_FILE_LOADER;
    if (!bytesReleaser) bytesReleaser = CSM_FALLBACK_BYTES_RELEASER;
#endif
