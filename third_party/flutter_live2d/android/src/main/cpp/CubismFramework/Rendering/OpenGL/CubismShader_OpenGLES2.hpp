...
private:
    ...
    /**
     * @brief   シェーダプログラムをコンパイルするのに必要な情報。
     *
     * ブレンドモードの組み合わせ分（16 × 5 × 6 = 480 個）を起動時に全部コンパイルすると
     * 実機で 4 秒以上かかる。ここには登録だけしておいて、実際に使う時にコンパイルする。
     */
    struct ShaderSpec
    {
        const csmChar* Vert;    ///< 頂点シェーダのパス（nullptr なら未登録）
        const csmChar* Frag;    ///< フラグメントシェーダのパス
        csmInt32 ColorBlend;    ///< ブレンドカラーモード
        csmInt32 AlphaBlend;    ///< オーバーラップカラーモード
        MaskType Mask;          ///< マスクの種類
        csmBool IsBlend;        ///< ブレンドモードを使用するか
    };
    ...
    */
    ...
    /**
     * @brief   シェーダーのコンパイルに必要な情報を登録する（コンパイルはしない）
     */
    void RegisterShader(csmInt32 index, const csmChar* vert, const csmChar* frag,
                        csmInt32 colorBlend, csmInt32 alphaBlend,
                        MaskType mask, csmBool isBlend);

    /**
     * @brief   シェーダプログラムを取得する。未コンパイルならこの時点でコンパイルする。
     */
    CubismShaderSet* GetShaderSet(csmInt32 index);
...
private:
    ...
    csmVector<ShaderSpec> _shaderSpecs;        ///< 遅延コンパイル用の登録情報
